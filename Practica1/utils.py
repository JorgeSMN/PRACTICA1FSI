...
class Ramificacion(Queue):
    ...
    def extend(self,items):
        self.A.extend(items)
        self.A.sort(key=lambda item: item.path_cost,reverse=False) ##ORDENAR POR EL COSTE


class RamificacionSubestimacion(Queue):
    ...
    def extend(self,items):
        self.A.extend(items)
        self.A.sort(key=lambda item: item.path_cost + self.problem.h(item),reverse=False) ##ORDENAR POR EL COSTE Y LA HEURISTICA
