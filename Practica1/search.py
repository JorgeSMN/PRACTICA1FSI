...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            """Si ha llegado al final, imprime los nodos expandidos"""
            print "Numero total de nodos expandidos:", nodos_expandidos
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))
            """Incrementa nodos expandidos"""
            nodos_expandidos = nodos_expandidos + 1
            """Imprime el nodo expandido"""
            print "Nodo expandido", nodos_expandidos, ": ", node
    return None

def ramificacion_search(problem):
    return graph_search(problem,Ramificacion())

def ramificacion_subestimacion_search(problem):
    return graph_search(problem,RamificacionSubestimacion(problem))
